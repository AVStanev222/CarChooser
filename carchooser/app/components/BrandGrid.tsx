...
export default function BrandGrid({ variant = "home" }: BrandGridProps) {
  ...
  return (
    <section className="relative bg-section pb-16 pt-4">
      <div className="max-w-6xl mx-auto px-6">
        <SectionTitle>
          Explore <span className="text-purple-400">Popular Brands</span>
        </SectionTitle>

        <p className="text-center text-muted max-w-2xl mx-auto mb-10 text-sm md:text-base">
          ...
          {visibleBrands.map((brand) => (
            <Link key={brand.name} href={brand.href} className="block group">
              <motion.div
                ...
                className="bg-card group-hover:bg-card-hover rounded-xl border border-border shadow-lg
                           flex flex-col items-center justify-center py-5 px-3 gap-3 cursor-pointer"
              >
                <div className="h-16 w-16 rounded-full bg-white/95 border border-border flex items-center justify-center overflow-hidden shadow-inner">
                  {brand.logo ? (
                    <Image
                      src={brand.logo}
                      alt={`${brand.name} logo`}
