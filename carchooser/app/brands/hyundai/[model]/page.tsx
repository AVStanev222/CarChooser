  ...
  return (
    <main className="min-h-screen bg-[#050214] text-white">
      <section className="max-w-5xl mx-auto px-6 pt-16 pb-20">
        <BackLink href="/brands/hyundai">Back to Hyundai cars</BackLink>
        <SectionTitle>
          Hyundai {car.model} <span className="text-purple-400">{car.year}</span>
        </SectionTitle>
        <p className="text-gray-300 max-w-3xl mb-10">{car.summary}</p>
        <CarActivityTracker
          carLabel={`Hyundai ${car.model} ${car.year}`}
          carPath={`/brands/hyundai/${car.slug}`}
        />

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-12">
          <VehicleImageGallery brand="Hyundai" model={car.model} year={car.year} slug={car.slug} />
        ...
        <div className="bg-[#0d071d] border border-purple-500/20 rounded-2xl p-6 flex flex-col gap-4">
          <h3 className="text-xl font-semibold">Hyundai ownership notes</h3>
          <p className="text-gray-300 text-sm leading-relaxed">
            Once the Supabase bridge is live you can save builds, log maintenance,
            and compare real-world prices for each Hyundai chassis you explore.
          </p>
        </div>
      </section>
    </main>
