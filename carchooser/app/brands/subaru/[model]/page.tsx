  ...
  return (
    <main className="min-h-screen bg-[#050214] text-white">
      <section className="max-w-5xl mx-auto px-6 pt-16 pb-20">
        <BackLink href="/brands/subaru">Back to Subaru cars</BackLink>
        <SectionTitle>
          Subaru {car.model} <span className="text-purple-400">{car.year}</span>
        </SectionTitle>
        <p className="text-gray-300 max-w-3xl mb-10">{car.summary}</p>
        <CarActivityTracker
          carLabel={`Subaru ${car.model} ${car.year}`}
          carPath={`/brands/subaru/${car.slug}`}
        />

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-12">
          <VehicleImageGallery brand="Subaru" model={car.model} year={car.year} slug={car.slug} />
        ...
        <div className="bg-[#0d071d] border border-purple-500/20 rounded-2xl p-6 flex flex-col gap-4">
          ...
          <p className="text-gray-300 text-sm leading-relaxed">
            Soon you will be able to store Supabase-backed notes, upload real
            photos, and compare insurance or financing for every Subaru model.
            For now, these placeholders show where galleries and owner tips will
            live.
          </p>
        </div>
      </section>
