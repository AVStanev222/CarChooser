  ...
  return (
    <main className="min-h-screen bg-page text-text transition-colors">
      <section className="max-w-5xl mx-auto px-6 pt-16 pb-20">
        ...
        <CarActivityTracker
          carLabel={`Opel ${car.model} ${car.year}`}
          carPath={`/brands/opel/${car.slug}`}
        />

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-12">
          <VehicleImageGallery brand="Opel" model={car.model} year={car.year} slug={car.slug} />
        ...
        <div className="bg-card-muted border border-purple-500/20 rounded-2xl p-6 flex flex-col gap-4">
          <h3 className="text-xl font-semibold">Opel ownership notes</h3>
          <p className="text-muted text-sm leading-relaxed">
            Supabase integration will unlock build-saving, service logs, and
            market tracking for every Opel you explore here.
          </p>
        </div>
      </section>
    </main>
