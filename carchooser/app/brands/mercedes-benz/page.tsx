  ...
  return (
    <main className="min-h-screen bg-page text-text transition-colors">
      <section className="max-w-6xl mx-auto px-6 pt-16 pb-12">
        ...
        <SectionTitle>
          Mercedes-Benz <span className="text-purple-400">Collection</span>
        </SectionTitle>
        <p className="text-muted max-w-3xl mb-10">
          Dive into compact A-Class hatchbacks, stately S-Class limos, and the
          wildest AMG creations. Tap any chassis to see curated specs today and
          richer data once our Supabase sync launches.
        </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {mercedesModels.map((model) => (
            <Link
              key={model.slug}
              href={`/brands/mercedes-benz/${model.slug}`}
