...
export default function BentleyModelsPage() {
  return (
    <main className="min-h-screen bg-[#050214] text-white">
      <section className="max-w-6xl mx-auto px-6 pt-16 pb-12">
        <SectionTitle>
          Bentley <span className="text-purple-400">Lineup</span>
        </SectionTitle>
        <p className="text-gray-300 max-w-3xl mb-10">
          Browse handcrafted GTs, limousines, and SUVs from Crewe. Every tile
          leads to specifications, pricing history, and gallery placeholders.
        </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {bentleyModels.map((model) => (
            <Link
              key={model.slug}
              href={`/brands/bentley/${model.slug}`}
