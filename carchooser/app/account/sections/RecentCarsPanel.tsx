      ...
      <div className="mt-4 space-y-3">
        {cars.length ? (
          cars.map((entry) => (
            <div
              key={entry.path}
              className="flex items-center justify-between rounded-2xl border border-white/5 bg-white/5 px-4 py-3 text-white/90"
            >
              <Link
                href={entry.path}
                className="flex-1 text-white transition hover:text-emerald-300"
              >
                {entry.label}
              </Link>
              <button
                ...
                className="ml-3 rounded-full border border-white/20 px-3 py-1 text-xs uppercase tracking-wide text-white/70 transition hover:border-red-400 hover:text-red-300 disabled:cursor-not-allowed disabled:opacity-50"
              >
